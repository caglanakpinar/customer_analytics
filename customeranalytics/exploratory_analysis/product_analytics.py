...
class ProductAnalytics(BaseEDA):
    """

    """
    def __init__(self, has_product_connection=True, host=None, port=None, download_index='downloads',
                 order_index='orders'):
        """
        !!!!
        ******* ******** *****
    ...
    def get_products(self, end_date):
        ...
        self.products['session_start_date'] = self.products['session_start_date'].apply(self.convert_to_date)
    ...
    def get_customer_segments(self, date):
        ...
        """
        date = self.current_date_to_day().isoformat() if date is None else date
        self.products = pd.merge(self.products,
                                 self.cs.fetch(start_date=self.convert_dt_to_day_str(date))[['client', 'segments']],
                                 on='client', how='left')
    ...
    def get_hourly_products_of_sales(self):
        ...
        """
        self.products['hours'] = self.products['session_start_date'].apply(self.convert_str_to_hour)
    ...
    def get_daily_product_sales(self):
        ...
        """
        self.products['daily'] = self.products['session_start_date'].apply(self.convert_dt_to_day_str)
    ...
    def execute_product_analysis(self, end_date=None):
        ...
        if self.has_product_connection:
            self.get_products(end_date=self.convert_to_iso_format(self.current_date_to_day() if end_date is None else end_date))
